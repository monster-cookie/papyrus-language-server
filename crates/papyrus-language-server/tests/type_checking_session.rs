...
fn expected_diagnostic_lines() -> BTreeMap<String, Vec<u64>> {
    BTreeMap::from([
        ...
        ("invalid-array-size".to_owned(), vec![20]),
        ("invalid-assignment-target".to_owned(), vec![12]),
        ("invalid-binary-operands".to_owned(), vec![16, 24]),
        ("invalid-cast".to_owned(), vec![21]),
        ("invalid-compound-assignment".to_owned(), vec![13, 14]),
        ("invalid-condition".to_owned(), vec![23]),
        ("invalid-subscript-index".to_owned(), vec![17]),
        ("invalid-subscript-target".to_owned(), vec![18]),
        ("invalid-type-test".to_owned(), vec![22]),
        ("invalid-unary-operand".to_owned(), vec![15]),
        ("missing-return-value".to_owned(), vec![7]),
        ("unexpected-return-value".to_owned(), vec![26]),
        ("void-value-use".to_owned(), vec![19]),
    ])
}
